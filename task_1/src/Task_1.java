...
// —--------------------------------------------------------------------------------—
        double up_side=left_up_x+width, l_side=left_up_y-height;
        if (width == height){
            if ((x-left_up_x<=0.000001) || (up_side-x<=0.000001) || (left_up_y-y<=0.000001) || (y-l_side<=0.000001))
            { return 0; }
            else if ((x >left_up_x && x<up_side) ||(y >left_up_y && y<l_side)) { return 1; }
            else { return 0; }
        } else { return 2; }
    }
    @Override
...
// Допустимой погрешностью при сравнении переменных типа double считать 0.000001
// —--------------------------------------------------------------------------------—
        if (Math.abs(y0 - (k*x0 + b)) <= 0.000001) {return 2;}
        if (y0 > k*x0 + b) {return 0;}
        else return 1;
    }
    @Override
...
// —--------------------------------------------------------------------------------—
        String weekday = switch(day_od_week){
            case 1 -> "Понедельник";
            case 2 -> "Вторник";
            case 3 -> "Среда";
            case 4 -> "Четверг";
...
// 2 - аргументы функции заданы некорректно
// Допустимой погрешностью при сравнении переменных типа double считать 0.000001
        double ball_way = Math.abs(wall) * Math.sqrt( vx*vx + vy*vy+ vz*vz );
        if ((speed>0 && time>0 && wall!=0 )){
            if ( (Math.abs(ball_way/speed-time) <= 0.000001) || (ball_way/speed<=time) && (vx!=0))  return 1;
                else return 0; }
        else return 2;
...
// Допустимой погрешностью при сравнении переменных типа double считать 0.000001
// —--------------------------------------------------------------------------------—
        if ((Math.abs(k1 - k2) <= 0.000001) && (k1 == k2)) {
            if (b1 != b2) { return 1; }
            else return 3;}
        else return 2; }
}